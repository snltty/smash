...
namespace smash.proxy.server
{
    ...
    {
        private ConcurrentDictionary<IPEndPoint, UdpServerUserToken> udpConnections = new(new ConnectionKeyUdpComparer());
        private readonly SemaphoreSlim Semaphore = new SemaphoreSlim(1);

        private readonly WheelTimer<object> wheelTimer = new WheelTimer<object>();
        ...
        public async Task InputData(AsyncServerUserToken token)
        {
            if (token.Step == Socks5EnumStep.Request)
            {
                await Command(info);
            }
            ...
            else if (info.Step == Socks5EnumStep.ForwardUdp)
            {
        ...
        private async Task Command(ProxyInfo info)
        {
            try
            {
                ...
                else if (info.Command == Socks5EnumRequestCommand.UdpAssociate)
                {
                    await ConnectReponse(info, Socks5EnumResponseCommand.ConnecSuccess);
                }
                else if (info.Command == Socks5EnumRequestCommand.Bind)
                {
                    await ConnectReponse(info, Socks5EnumResponseCommand.CommandNotAllow);
                }
                else
                {
                    await ConnectReponse(info, Socks5EnumResponseCommand.CommandNotAllow);
            ...
            catch (Exception ex)
            {
                ...
                await ConnectReponse(info, Socks5EnumResponseCommand.ConnectFail);
                return;
            }
        ...
        private async Task ForwardUdp(ProxyInfo info)
        {
            ...
            try
            {
                if (udpConnections.TryGetValue(key, out UdpServerUserToken token) == false)
                {
                    ...
                    token = new UdpServerUserToken { Data = info, TargetSocket = socket, Key = key, TargetEP = remoteEndpoint };
            ...
            {
                if (udpConnections.TryRemove(key, out UdpServerUserToken _token))
                {
                    _token.Clear();
                }
        ...
        {
            UdpServerUserToken token = result.AsyncState as UdpServerUserToken;
            try
            {
        ...
        private async Task ConnectReponse(ProxyInfo info, Socks5EnumResponseCommand command)
        {
    ...
    public sealed class UdpServerUserToken
    {
        public IPEndPoint Key { get; set; }
        public Socket TargetSocket { get; set; }
        public ProxyInfo Data { get; set; }
        public byte[] PoolBuffer { get; set; }
    ...
    public sealed class ConnectionKeyUdpComparer : IEqualityComparer<IPEndPoint>
    {
